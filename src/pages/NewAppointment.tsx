...
import { createAppointment, Recurrence } from "../services/appointment.service"
...
export function NewAppointment() {
  ...
  const [currentMonth, setCurrentMonth] = useState<string>("") // YYYY-MM format
  ...
  useEffect(() => {
    const fetchCalendar = async () => {
      if (!calendarId) {
        toast.error("No calendar specified")
        navigate("/calendars")
        return
      }
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          // Auto-select the current date or the calendar's live date if in the future
          const today = new Date().toISOString().split("T")[0]
          const liveDate = response.data.liveAt.split("T")[0]
          const expireDate = response.data.expireAt.split("T")[0]

          let initialDate = today
          if (liveDate > today) {
            initialDate = liveDate
          }
          if (initialDate > expireDate) {
          ...
          // Set initial month for slots fetching
          setCurrentMonth(initialDate.substring(0, 7)) // YYYY-MM
        } else {
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      ...
      try {
        ...
        // Calculate first and last day of the month using date-fns
        const monthDate = parseISO(`${currentMonth}-01`) // Parse YYYY-MM-01
        ...
        const response = await getCalendarSlots(calendarId, {
          liveAt,
          expireAt,
        })
        const booked = response.data?.data.filter(d => d.booked)
        console.log(booked)
  ...
  useEffect(() => {
    if (formData.date) {
      const newMonth = formData.date.substring(0, 7) // YYYY-MM
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!ValidateAppointment(formData, setErrors) || !calendarId || !calendar) return
    ...
    try {
      // For one-time appointments, extract day and month from the selected date
      let days: number[] = []
      let months: number[] = []
      let liveAt: string
      let expireAt: string
      ...
      if (formData.recurrence === Recurrence.UNSPECIFIED) {
        // One-time appointment: extract date details using date-fns
        const selectedDate = parseISO(formData.date)
        const dayOfMonth = getDate(selectedDate) // Day of month (1-31)
        ...
        // Recurring appointment: use selected days/months and calendar's availability window
        days = formData.days
  ...
  return (
    <div className="min-h-screen bg-[#F7F9FC] flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto px-4 py-12 mt-16 w-full">
        <Link
          to={calendarId ? `/calendars/${calendarId}` : "/calendars"}
        ...
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-[#1A1A1A]">Book Appointment</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-xl shadow-sm border border-[#E5EAF2] p-6" noValidate>
          ...
          {/* Date Selection - only show for one-time appointments */}
          ...
          {/* Months Selection - Both Weekly and Monthly */}
